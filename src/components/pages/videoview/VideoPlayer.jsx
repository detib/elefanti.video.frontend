...
const VideoPlayer = () => {
  ...
  const fetchVideo = async () => {
    await axios
      ...
      .then((response) => {
        response.data.createdOn = format(new Date(response.data.createdOn), 'd MMM yyyy');
  ...
  const likeVideo = async (e) => {
    if (!context.data.isLoggedIn) {
      e.preventDefault();
      toast.warning('You need to be logged in to like videos.', { autoClose: 3500 });
      return;
    }

    if (e.target.checked) {
      await axios
        .post(`${process.env.REACT_APP_API}/api/reactions/likes/${videoId}`, null, {
          headers: {
      ...
      await axios
        ...
        .then(() => {
          ...
          const likeToBeRemoved = videoLikes.findIndex((l) => l.UserId == jwtDecode(context.data.token).Id);
  ...
  return (
    <div className='content'>
      <div className='video-player-wrapper'>
        ...
        <div className='comment-section'>{video.id && <CommentSection videoId={videoId} />}</div>
      </div>
      <div className='suggestedVideos'>{video.id && <SuggestedVideos videoId={videoId} />}</div>
    </div>
  );
};
